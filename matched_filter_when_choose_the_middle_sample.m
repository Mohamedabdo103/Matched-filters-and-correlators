...
for snr = 0:2:30
  ...
  %------------------------- matched filter -------------------------------
    ...
    mf_output = zeros(1,number_of_bits);
    conv_output = zeros(1,(2*20-1)*number_of_bits);
    ...
    for i = 0 : number_of_bits -1
        noisyWF_sample = noisyWF((i*20)+1:(i+1)*20);  %Extracting 20 samples
        mf_sample = conv(noisyWF_sample,h_mf);
        
        %Concatenating the conv results
        conv_output( (length(h_mf)+length(noisyWF_sample)-1)*i+1:(length(h_mf)+length(noisyWF_sample)-1)*i+length(mf_sample) ) = mf_sample;  
        mid_point(i+1) = 20 + (length(h_mf)+length(noisyWF_sample)-1)*(i);    %middle sample index
        
        
        mf_output(i+1) = conv_output(mid_point(i+1));  %concatenating the middle sample to the o/p  
    end
